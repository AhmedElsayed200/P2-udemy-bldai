...
const CourseHeader = ({ selectedCourseSummary }) => {
  return (
    <>
        ...
        >
          ...
          <div id="lastUpdated" className="flex gap-x-3 items-center mb-2">
            ...
            {`${Number(
              ...
              selectedCourseSummary?.last_update_date?.split("-")[0]
            )}`}
          </div>
      ...
      <Navigation
        title={selectedCourseSummary?.title}
        rate={selectedCourseSummary?.rating}
        nOfRaters={selectedCourseSummary?.num_reviews}
        nOfStudents={selectedCourseSummary?.num_subscribers}
      />
    </>
  );
};
